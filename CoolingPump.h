...
public:
    ...
    void tick() {
        ...
        // Safety: stop controller if exceeded max continuous daily runtime
        if (_totalPumpMs >= cfg.MAX_CONT_MS) {
            _pumpOff(); _enabled = false;
            pushMsg("Cooling disabled – 12‑h limit reached");
            return;
        }
    ...
    struct Cfg {
        ...
        // Duty‑cycle timing (save motor)
        uint32_t MIN_RUN_MS      = 3UL * 60UL * 1000UL;   // 3 minutes on
        uint32_t OFF_PAUSE_MS    = 1UL * 60UL * 1000UL;   // 1 minute off
        // Safety cap
    ...
    case State::BURST:
        // Mid‑burst notification every 30 min
        if(now - _lastInterStatsMs >= INTER_STATS_MS) {
            char midBuf[80];
            double dropSoFar = _lastAquaStartF - _tAquaF;
            snprintf(midBuf, sizeof(midBuf), "Burst %lu min: Aquarium↓%.2f°F (%.1f→%.1f)", (now - _burstStart)/60000UL, dropSoFar, _lastAquaStartF, _tAquaF);
            pushMsg(midBuf);
            _lastInterStatsMs = now;
        }
        // Stop after fixed 3‑minute window OR coil/temperature criteria
        if(now - _burstStart >= cfg.MIN_RUN_MS ||
           (_tOutletF - _tInletF <= cfg.dT_STOP_COIL) ||
           (_tAquaF <= cfg.T_LOW)) {
            _pumpOff();
            _totalPumpMs += now - _burstStart;
            _state = State::LOCK; _lastStop = now;
            ...
            snprintf(endBuf, sizeof(endBuf), "Burst end: Aquarium↓%.2f°F (%.1f→%.1f), Bath↑%.2f°F", drop, _lastAquaStartF, _lastAquaEndF, _lastBathEndF - _lastBathStartF);
